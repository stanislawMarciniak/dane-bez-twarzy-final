...
3. Zapis do outputOverfitters.txt
4. Detailed Labels - dodaje info o płci i przypadku
5. Synthetic Generator - generuje dane syntetyczne
6. Zapis do synthetic_generation.txt

Zawiera szczegółowe mierzenie czasu dla każdej warstwy.
"""
...
OUTPUT_SYNTHETIC = "synthetic_generation.txt"
...
class TimingResult:
    ...
    time_to_synthetic: float = 0.0   # Do synthetic_generation.txt
...
║                    ⏱️  POMIAR CZASU                          ║
╠══════════════════════════════════════════════════════════════╣
║ Ładowanie modelu ML:           {self.model_load_time:>8.3f} s                  ║
║ Warstwa ML (NER):              {self.ml_layer_time:>8.3f} s                  ║
║ Warstwa Regex:                 {self.regex_layer_time:>8.3f} s                  ║
║ Detailed Labels:               {self.detailed_labels_time:>8.3f} s                  ║
║ Generacja syntetyczna:         {self.synthetic_generation_time:>8.3f} s                  ║
║ Zapis plików (I/O):            {self.file_io_time:>8.3f} s                  ║
╠══════════════════════════════════════════════════════════════╣
║ 📄 Czas do outputOverfitters:  {self.time_to_anonymized:>8.3f} s                  ║
║ 📄 Czas do synthetic_gen:      {self.time_to_synthetic:>8.3f} s                  ║
╠══════════════════════════════════════════════════════════════╣
║ 🏁 CAŁKOWITY CZAS:             {self.total_time:>8.3f} s                  ║
╚══════════════════════════════════════════════════════════════╝
"""
...
class AnonymizationPipeline:
    ...
    def __init__(self, model_path: str = MODEL_PATH, verbose: bool = True):
    ...
    def load_models(self):
        """Ładuje model ML i inicjalizuje warstwę regex."""
        ...
        self._log(f"✅ Model ML załadowany ({self.timing.model_load_time:.3f}s)")
    ...
    def process(
        ...
        self._log(f"\n💾 Zapisuję do {output_anonymized}...")
        with open(output_anonymized, 'w', encoding='utf-8') as f:
            f.write(after_regex)
        t_io_anon = time.perf_counter() - t_io_start
        
        # Czas do outputOverfitters.txt
        self.timing.time_to_anonymized = time.perf_counter() - pipeline_start
        self._log(f"✅ Zapisano: {output_anonymized}")
        ...
        # === ZAPIS DO synthetic_generation.txt ===
        t_io_start = time.perf_counter()
        self._log(f"\n💾 Zapisuję do {output_synthetic}...")
        ...
        self._log(f"✅ Zapisano: {output_synthetic}")
...
def main():
    ...
    print("   📊 Z mierzeniem czasu dla każdej warstwy")
    print("="*60)
